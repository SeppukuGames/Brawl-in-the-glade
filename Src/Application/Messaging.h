...
class Mensaje {
public:
	TipoMensaje m_TipoMensaje;
	int m_Id;
	Mensaje(TipoMensaje tipo, int id) : m_TipoMensaje(tipo), m_Id(id) {
	
	}
	~Mensaje();
};

class MensajeFisico : public Mensaje {
public:
	MensajeFisico(int id) : Mensaje(MSG_FISICO, id) {

	}
	~MensajeFisico();
};

class MensajeLogico : public Mensaje {
public:
	MensajeLogico(int id) : Mensaje(MSG_LOGICO, id) {

	}
	~MensajeLogico();
};

class BaseDeMensajes {
	// Voy a definir un tipo que puede ser cualquier cosa en memoria
	typedef void(*Encargado)(BaseDeMensajes*, const Mensaje&);

	struct Union {
		TipoMensaje m_tipo;
		BaseDeMensajes * m_Observer;
		BaseDeMensajes * m_Observed;
		Encargado m_Encargado;
		Union(TipoMensaje tipo, BaseDeMensajes * Observer,
			BaseDeMensajes * Observed, Encargado Encargado) :
			m_tipo(tipo), m_Observer(Observer),
			m_Observed(m_Observed), m_Encargado(Encargado) {

		}
	};

	std::vector<Union> m_Uniones;
