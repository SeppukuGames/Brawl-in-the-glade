...
button::button(const std::string &buttonName, const std::string &material, const buttonPosition &position, short width, short height, buttonManager * mgr, Ogre::Overlay * o, button * parentButton, textScheme &style, bool isActiveButton) :
name(buttonName),
buttonMgr(mgr),
overlay(o),
parent(parentButton),
...
button * button::createChildButton(std::string name, std::string material, const buttonPosition &position, short  width, short  height, bool isActiveButton)
{
	return createChildButton(name, material, position, width, height, textStyle, isActiveButton);
}
button * button::createChildButton(std::string name, std::string material, const buttonPosition &position, short  width, short  height, textScheme &scheme, bool isActiveButton)
{
...
relY(positionY),
zoom(1),
objectAngle(0.9273f), //this is the radian angle that an object of height 1 @ distance 1 would obscure from a point
rotatableX(true),
rotatableY(true),
mouseOn(false),
momentumX(0),
...
buttonMesh * buttonMesh::setDimensions(short w, short h)
{
	...
	screenSpaceModifier = (1.0f / float(buttonMgr->screenResY));
...
grabbedButton(NULL),
grabbingMouseButton(0),  //defnes the mouse button that is used to drag buttons around
turningMouseButton(1), // defines the mouse button that can rotate buttonMeshes
zOrderCounter(5),
activeTextInputArea(NULL),
eventContainer(ONSUBMIT, -1, NULL, NULL),  //junk event container just for the initialization
capslock(false),
lshift(false),
rshift(false),
backSpace(false),
...
button * buttonManager::setCursor(std::string material, unsigned short width, unsigned short height, unsigned short hotspotX, unsigned short hotspotY, bool visibility)
{
	...
	buttonPosition p(mouseX - mouseOffsetX, mouseY - mouseOffsetY);
	cursorButton->setPosition(p);
...
{
	if (arg.key == OIS::KC_TAB)  //cycle through tia's
	{
		cycleTextInputArea();
	}

	if (arg.key == OIS::KC_CAPITAL)  //caps lock
	{
		if (capslock)
			capslock = false;
		else
	...
	{ //inject the key into the active textArea 
		if (arg.key == OIS::KC_RETURN)
		{
			...
			{
				eventLog.push_back(buttonEvent(ONSUBMIT, -1, activeTextInputArea, NULL, *activeTextInputArea->getValue())); //log the event
		...
		else if (arg.key == OIS::KC_BACK)
		{
			if (activeTextInputArea->insertBackspace())
				eventLog.push_back(buttonEvent(KEYACCEPTED, -1, activeTextInputArea, NULL, "BACKSPACE")); //log the event, accepted
			else
				eventLog.push_back(buttonEvent(KEYREJECTED, -1, activeTextInputArea, NULL, "BACKSPACE")); //log the event as rejected
			backSpace = true;
		...
		else
		{
			...
			if (accepted)
				eventLog.push_back(buttonEvent(KEYACCEPTED, -1, activeTextInputArea, NULL, eventStringContainer)); //log the event
			else
				eventLog.push_back(buttonEvent(KEYREJECTED, -1, activeTextInputArea, NULL, eventStringContainer)); //log the event
		}
	}
	return true;
}
...
bool buttonManager::injectMouseDown(const int id)
{
...
bool buttonManager::injectMouseUp(const int id)
{
	button* topButton = getTopButton();
...
bool buttonManager::injectMouseMove(int xPos, int yPos)
{
	...
	mouseX = xPos;
	mouseY = yPos;
	buttonPosition p(xPos - mouseOffsetX, yPos - mouseOffsetY);
	cursorButton->setPosition(p);  //update our cursor, line it up with trigger point.
...
void buttonManager::forceClick(const int mouseButton)
{
	injectMouseDown(mouseButton); //insert a single click
	injectMouseUp(mouseButton);
}

void buttonGUI::buttonManager::forceClickButton(button *b, const int mouseButton /*= MB_Left*/)
{
	if (b)
	{
		short origX = mouseX;
...
void buttonGUI::buttonManager::forceClickButton(std::string buttonName, const int mouseButton /*= MB_Left*/)
{
	forceClickButton(getButton(buttonName), mouseButton);
}

button* buttonManager::createButton(std::string buttonName, std::string material, const buttonPosition &position, unsigned short width, unsigned short height, unsigned short zOrder, bool visible, bool isActive)
{
	return createButton(buttonName, material, position, width, height, defaultTextStyle, zOrder, visible, isActive);
}

button* buttonManager::createButton(std::string buttonName, std::string material, const buttonPosition &position, unsigned short width, unsigned short height, textScheme &scheme, unsigned short zOrder, bool visible, bool isActive)
{
	if (!zOrder)
		...
		if (backSpace)
		{
			if (backSpaceHeld)
			{
				...
				if (timePassed != 0)
				{
					...
					{
						if (activeTextInputArea->insertBackspace())
							eventLog.push_back(buttonEvent(KEYACCEPTED, -1, activeTextInputArea, NULL, "BACKSPACE")); //log the event
						else
							eventLog.push_back(buttonEvent(KEYREJECTED, -1, activeTextInputArea, NULL, "BACKSPACE")); //log the event
					}
				}
			}
...
std::string  buttonManager::keyCodeToString(const OIS::KeyCode &key, bool shift, bool alphanumericOnly)
{
	...
	if (key == OIS::KC_1)
		...
		s = "0";
	else if (key == OIS::KC_MINUS) {  // - on main keyboard
		if (shift)	s = "_";
		else	s = "-";
	}
	...
	else if (key == OIS::KC_NUMPAD7)
		s = "7";
	else if (key == OIS::KC_NUMPAD8)
		s = "8";
	else if (key == OIS::KC_NUMPAD9)
		s = "9";
	else if (key == OIS::KC_SUBTRACT)// - on numeric keypad
		s = "-";
	else if (key == OIS::KC_NUMPAD4)
		s = "4";
	else if (key == OIS::KC_NUMPAD5)
		s = "5";
	else if (key == OIS::KC_NUMPAD6)
		s = "6";
	else if (key == OIS::KC_NUMPAD1)
		s = "1";
	else if (key == OIS::KC_NUMPAD2)
		s = "2";
	else if (key == OIS::KC_NUMPAD3)
		s = "3";
	else if (key == OIS::KC_NUMPAD0)
		s = "0";
	...
	if (!alphanumericOnly)
	{
		if (key == OIS::KC_SPACE)
			s = " ";
		else if (key == OIS::KC_LBRACKET){
			if (shift)	s = "{";
			else	s = "[";
		}
		else if (key == OIS::KC_RBRACKET){
			if (shift)	s = "}";
			else	s = "]";
		}
		else if (key == OIS::KC_PERIOD){
			if (shift)	s = ">";
			else	s = ".";
		}
		else if (key == OIS::KC_COMMA){
			if (shift)	s = "<";
			else	s = ",";
		}
		else if (key == OIS::KC_SEMICOLON){
			if (shift)	s = ":";
			else	s = ";";
		}
		else if (key == OIS::KC_SLASH){
			if (shift)	s = "?";
			else	s = "/";
		}
		else if (key == OIS::KC_SLASH){
		...
		else if (key == OIS::KC_BACKSLASH){
			if (shift)	s = "|";
			else	s = "/";
		}  //lets keep it forward slash,  just in case someone can use a backslash to exploit some parsing function.
		else if (key == OIS::KC_APOSTROPHE){
			if (shift)	s = "\"";
			else	s = "'";
		}
		else if (key == OIS::KC_EQUALS){
			if (shift)	s = "+";
			else	s = "=";
		}
		else if (key == OIS::KC_ADD) // + on keypad
			s = "+";
		else if (key == OIS::KC_NUMPAD0)
			s = "0";
		else if (key == OIS::KC_MULTIPLY)// * on numeric keypad
			s = "*";
		else if (shift)
		{
			if (key == OIS::KC_1)
